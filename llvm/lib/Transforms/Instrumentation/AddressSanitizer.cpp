...
{
  IRBuilder<> IRB(InsertBefore);
  PHINode* PHINd = IRB.CreatePHI(ArrayIdx->getType(),3);
  for(BasicBlock *pred : predecessors(InsertBefore->getParent())) {
    if(pred == RecoverBlock) {
      PHINd->addIncoming(ArrayIdx, RecoverBlock);
    }
    else {
  ...
  if(IsWrite) {
    ...
    for(Use &StoreU : InsertBefore->operands()) {
      ...
      if(GetElementPtrInst* Inst = dyn_cast<GetElementPtrInst>(v)) {
        OriginalArrayIdx = dyn_cast<Value>(Inst);
        for(Use &U : Inst->operands()) {
          Value *v = U.get();
          if(LoadInst* LI = dyn_cast<LoadInst>(v)) {
            if(PointerType* Ty = dyn_cast<PointerType>(LI->getPointerOperandType())) {
              if(Ty->getElementType()->isPointerTy()) {
                StartAddr = LI->getPointerOperand();              //get the start address of array
                break;
              }
            }
          } 
    ...
    if(index->getType() == Type::getInt64Ty(*C))
    {
    ...
    */
    ...
    if(ArraySize->getType() != Type::getInt64Ty(*C))
    {
      ArraySize = IRB.CreateSExt(ArraySize, Type::getInt64Ty(*C));
    }
    Type* ArrayElementTy = StartAddr->getType()->getPointerElementType()->getPointerElementType();
    ...
    if(ArrayAddr->getType() != Type::getInt8PtrTy(*C))
    {
      ArrayAddr = IRB.CreateBitCast(ArrayAddr, Type::getInt8PtrTy(*C));
    }
    
    FunctionCallee ReallocFunc = M->getOrInsertFunction("realloc", Type::getInt8PtrTy(*C), Type::getInt8PtrTy(*C), Type::getInt64Ty(*C));
    ...
    */
    ...
    if(index->getType() != Type::getInt64Ty(*C))
    {
       index = IRB.CreateSExt(index, Type::getInt64Ty(*C));
    }
...
void AddressSanitizer::instrumentAddress(Instruction *OrigIns,
 ...
 // Instruction *CrashTerm = nullptr;
  ...
  Instruction* RecoverTerm = nullptr;
  ...
  if (ClAlwaysSlowPath || (TypeSize < 8 * Granularity)) {
    ...
    */
    BasicBlock* RecoverBlock = BasicBlock::Create(*C, "", NextBB->getParent(), NextBB);
    RecoverTerm = BranchInst::Create(NextBB, RecoverBlock);
    BranchInst* NewTerm = BranchInst::Create(RecoverBlock, NextBB, Cmp2);
    ReplaceInstWithInst(CheckTerm, NewTerm);
    
  } else {
    //CrashTerm = SplitBlockAndInsertIfThen(Cmp, InsertBefore, !Recover);
    RecoverTerm = SplitBlockAndInsertIfThen(Cmp, InsertBefore, false);
  }
  
  Instruction* nextInst = InsertBefore->getNextNode();
  BasicBlock* nextBlock = NULL;                                       // to hold next block if we should skip the write
  if(IsWrite) {
    if(nextInst && !isa<BranchInst>(nextInst))
      nextBlock = InsertBefore->getParent()->splitBasicBlock(nextInst);
    else 
      nextBlock = InsertBefore->getParent()->getSingleSuccessor();
  }
  generateRecoverCode(RecoverTerm, nextBlock, InsertBefore, IsWrite);
